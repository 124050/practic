...
// Делаем глубокую копию для стартовых данных, чтобы не повредить initialProcesses
let processes = initialProcesses.map(function(p) {
...
function renderMatrix() {
  ...
  // Распределяем отфильтрованные процессы
  getFilteredProcesses().forEach(function(process) {
...
function handleAddProcess(event) {
  ...
  renderAll(); // Внутри уже вызывается saveToStorage()
  event.target.reset();
}
...
function sortProcesses(field) {
  ...
  renderAll(); // Сохранит измененный порядок в localStorage
}
